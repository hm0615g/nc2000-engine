...
class PolicyValue(nn.Module):
    ...
    def load_export(self, data):
        if data["schema"] != "nc2000-policy-value-v1" or data["vocabulary"] != self.vocabulary:
...
def evaluate(model, rows, batch_size):
    if not rows:
        return None
    total_loss = total_brier = correct = count = choice_count = choice_correct = 0
    model.eval()
    with torch.no_grad():
...
def main():
    ...
    training = {"seed": args.seed, "objective": "teacher_action_and_terminal_outcome", "torch_version": torch.__version__}
